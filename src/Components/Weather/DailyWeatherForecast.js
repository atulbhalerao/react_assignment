...
function DailyWeatherForecast(props)
{
    ...
    const [dailyWeather, setDailyWeather] = useState({ date : '', dayTemp : '', nightTemp : '', mainWeather : '', icon :''});
    useEffect(()=>{
        if(props.value.dt)
        {
            let objWeatherService =new WeatherService();
            setDailyWeather({ date: String(objWeatherService.unixTimestampToDate(props.value.dt).date),
